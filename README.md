# Serper Google Search Nikola 9 MCP Server

This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Serper Google Search Nikola 9 API. It enables AI agents and LLMs to interact with Serper Google Search Nikola 9 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Serper Google Search Nikola 9 endpoints
...
- **Serper Google Search Nikola 9 Website**: [https://google.serper.dev](https://google.serper.dev)
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/serper-google-search-nikola-9-mcp-server.git
   cd serper-google-search-nikola-9-mcp-server
   ```

2. Set your API key:
   ```bash
   export SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Serper Google Search Nikola 9 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Serper Google Search Nikola 9 API key
2. Server uses client's API key to call Serper Google Search Nikola 9 API
3. No payment required
4. Client pays Serper Google Search Nikola 9 directly
...
6. Server uses its INTERNAL Serper Google Search Nikola 9 API key to call the API
...
```bash
# Required
SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=your_internal_serper-google-search-nikola-9_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Serper Google Search Nikola 9 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Serper Google Search Nikola 9 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/serper-google-search-nikola-9-mcp-server",
  "mcp_server": {
    "name": "serper-google-search-nikola-9-mcp",
    "description": "Lightning-fast google search api - search, news, images, places, videos, shopping, scholar, and more via serper.dev",
...
"capabilities": [
  ...
  "tags": ["serper google search nikola 9", "api"],
  "ref": "main"
}
```
...
- `SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY`: Your Serper Google Search Nikola 9 API key (required)
## Troubleshooting
