...
# Script to build and run the Serper Google Search Nikola 9 MCP Server locally in Docker
...
IMAGE_NAME="serper-google-search-nikola-9-mcp-server"
CONTAINER_NAME="serper-google-search-nikola-9-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Serper Google Search Nikola 9 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Serper Google Search Nikola 9 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO

# API Authentication (server's internal key for payment mode)
SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=
...
if ! grep -q "^SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Serper Google Search Nikola 9 API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=.*|SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=.*|SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ SERPER_GOOGLE_SEARCH_NIKOLA_9_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ Serper Google Search Nikola 9 MCP Server is running!${NC}"
echo
